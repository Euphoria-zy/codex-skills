...
**REQUIRED SUB-SKILL:** Use `superpowers:brainstorming` for the dialogue while satisfying this Skill's product and usability outputs. The user resolves choices that materially change behavior.
...
**REQUIRED SUB-SKILL:** Use `superpowers:writing-plans` after design approval.
...
**REQUIRED SUB-SKILLS:** Use `superpowers:test-driven-development` for behavior changes and `coding-standards` for the minimal implementation. Use `superpowers:systematic-debugging` when behavior is unexpected.
